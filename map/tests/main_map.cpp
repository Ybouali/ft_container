...
int     main()
{
    // goto next;
    {
        std::string arrstr[64] = { "CBCId4lUU6oYms60JkNr", "V5sbXjlqYRAV1C1LgJbt", "TPcK94z2htaoK5hWpR1j",
        ...
        ft::pair<int, std::string> arr[64];
        std::pair<int, std::string> arrs[64];

        for (int i = 0; i < 64; i++)
            arr[i] = ft::make_pair(i, arrstr[i]);

        for (int i = 0; i < 64; i++)
            arrs[i] = std::make_pair(i, arrstr[i]);

        std::map<int, std::string> maps =  std::map<int, std::string> (arr, arr + 29);
        ft::map<int, std::string> map =  ft::map<int, std::string> (arr, arr + 29);
        // ft::map<int, std::string> map2;
        ft::map<int, std::string> map1;

        map1 = ft::map<int, std::string> (arr, arr + 64);

            // std::cout << map.size() << std::endl;
            std::cout << map1.size() << std::endl;
        for (ft::map<int, std::string>::iterator i = map1.begin(); i != map1.end(); i++)
            std::cout << "FIRST :: " << i->first << " SECONDE :: " << i->second << std::endl;
